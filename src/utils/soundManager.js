...
class SoundManager {
  constructor() {
    ...
    if (this.isLocalDev) {
      console.log(`🎵 SoundManager initialized (Development mode)`);
    }
    
    // Audio file mapping with authentic Windows 11 sounds
    this.audioFiles = {
      // System sounds - Authentic Windows 11 audio
      startup: ['/audio/startup.wav', '/audio/windows-startup.m4a', '/audio/switch.mp3'],
      windowsStartup: ['/audio/windows-startup.m4a', '/audio/startup.wav'], // New Windows startup sound
      shutdown: ['/audio/shutdown.wav', '/audio/shutdown.mp3'],
      sleep: ['/audio/sleep.mp3'],
      
      // Window management - Real Windows 11 sounds
      windowOpen: ['/audio/window-open.wav', '/audio/switch.mp3'],
      windowClose: ['/audio/window-close.wav', '/audio/shutdown.mp3'],
      minimize: ['/audio/minimize.wav', '/audio/sleep.mp3'],
      maximize: ['/audio/maximize.wav', '/audio/switch.mp3'],
      
      // UI interactions - Authentic Windows 11 UI sounds
      buttonClick: ['/audio/button-click.wav', '/audio/switch.mp3'],
      hover: ['/audio/hover.wav', '/audio/sleep.mp3'],
      typing: ['/audio/typing.wav', '/audio/switch.mp3'],
  ...
  this.initializeSounds();
    ...
    this.volumeLevels = {
      master: 0.6,
      ambient: 0.3, // Reduced from 0.5 to 0.3 for better balance
      spotify: 0.7,
      effects: 0.5
    };
    
    // Track playing system sounds to prevent overlap
      ...
      Object.values(this.sounds).forEach(sound => {
        if (sound && typeof sound.pause === 'function') {
          try {
            ...
            if (this.isLocalDev) console.warn('⚠️ Failed to pause sound during reinitialize', e);
          }
        }
      });
      this.sounds = {};
  ...
  createSoundWithFallback(soundName, urls) {
    ...
    const tryNextUrl = (index = 0) => {
      if (index >= urls.length) {
        if (isDev) console.warn(`⚠️ All audio sources failed for ${soundName}`);
        if (soundName === 'ambientMusic') {
          if (isDev) console.log('🎵 Creating synthetic Comfort Chain...');
      ...
      const loadTimeout = setTimeout(() => {
        if (isDev) console.warn(`⚠️ Load timeout for ${soundName}, trying next...`);
        tryNextUrl(index + 1);
      }, 3000); // 3 second timeout (reduced from 5)

      audio.addEventListener('loadeddata', () => {
        if (isDev) console.log(`✅ Audio loaded: ${soundName}`);
      ...
      audio.addEventListener('error', (e) => {
        if (isDev) console.warn(`⚠️ Failed to load ${soundName}, trying next...`);
        clearTimeout(loadTimeout);
        tryNextUrl(index + 1);
      });
  ...
  configureSoundSettings(soundName, audio) {
    if (soundName === 'ambientMusic') {
      ...
      if (this.pendingAmbientStart && !this.isMuted) {
        if (this.isLocalDev) console.log('🎵 Ambient ready, starting queued playback');
        this.pendingAmbientStart = false;
        this.startAmbient();
      }
    } else {
    ...
    try {
      ...
      const syntheticAudio = {
        play: () => {
          ...
          isPlaying = true;
          if (this.isLocalDev) console.log('🎵 Playing synthetic Comfort Chain');
        ...
        pause: () => {
          if (currentSource && isPlaying) {
            ...
            currentSource = null;
            isPlaying = false;
            if (this.isLocalDev) console.log('⏸️ Paused synthetic Comfort Chain');
          }
        },
  ...
  play(soundName) {
    const isDev = this.isLocalDev;
    if (this.isMuted) {
      if (isDev) console.log(`🔇 Sound ${soundName} not played: audio is muted`);
      return;
    }
    
    if (!this.sounds[soundName]) {
      if (isDev) console.log(`🔇 Sound ${soundName} not available (still loading or failed)`);
      return;
    }
    ...
    try {
      ...
      if (soundName === 'ambientMusic' && sound && sound.paused === false && (typeof sound.currentTime !== 'number' || sound.currentTime > 0)) {
        if (isDev) console.log('🎵 Ambient music already playing, skipping restart');
        this.ambientStarted = true;
        return;
      }
      
      // Prevent overlapping Windows system sounds (but allow ambient music to continue)
      const isSystemSound = ['startup', 'windowsStartup', 'windowOpen', 'windowClose', 'minimize', 'maximize', 'shutdown'].includes(soundName);
      
      if (isSystemSound) {
        // Stop any currently playing system sounds to prevent overlap
        this.playingSounds.forEach(playingSound => {
          if (this.sounds[playingSound] && !this.sounds[playingSound].paused) {
            this.sounds[playingSound].pause();
            this.sounds[playingSound].currentTime = 0;
          }
        });
      ...
      if (typeof sound.currentTime === 'number') {
        if (soundName === 'ambientMusic') {
          if (sound.paused) {
            sound.currentTime = 0;
          }
      ...
      // Reduced ambient music volume from 0.8 to 0.3 for better balance
      sound.volume = this.masterVolume * (soundName === 'ambientMusic' ? 0.3 : 0.5);
      if (isDev) console.log(`🔊 Playing sound: ${soundName} at volume ${sound.volume}`);
      sound.play().then(() => {
        if (soundName === 'ambientMusic') {
          this.ambientStarted = true;
        }
        if (isDev && soundName === 'ambientMusic') {
        ...
        // Remove from playing sounds when system sound ends
        if (isSystemSound) {
      ...
      if (isDev) console.log('Sound playback error:', error);
    }
  }
  ...
  startAmbient() {
    if (this.isLocalDev) console.log('🎵 Attempting to start Comfort Chain...');

    if (this.isMuted) {
      if (this.isLocalDev) console.log('🔇 Ambient music not started: audio is muted');
      this.pendingAmbientStart = false;
      return;
    }

    const ambient = this.sounds.ambientMusic;

    if (!ambient) {
      if (this.isLocalDev) console.log('⌛ Ambient music not ready yet, queuing start');
      this.pendingAmbientStart = true;
      return;
    }

    if (ambient.paused === false && (typeof ambient.currentTime !== 'number' || ambient.currentTime > 0)) {
      if (this.isLocalDev) console.log('🎵 Ambient already playing, no restart needed');
      this.pendingAmbientStart = false;
      this.ambientStarted = true;
      return;
    }
  ...
  pauseAmbientForExternal() {
    if (this.sounds.ambientMusic && !this.sounds.ambientMusic.paused) {
      ...
      if (this.isLocalDev) console.log('🎵 Comfort Chain paused for external audio (Spotify)');
    }
  }
  ...
  resumeAmbientFromExternal() {
    if (this.sounds.ambientMusic && this.ambientWasPaused && !this.isMuted) {
      this.sounds.ambientMusic.play().catch(e => {
        if (this.isLocalDev) console.log('Ambient resume prevented:', e);
      });
      this.ambientWasPaused = false;
      this.ambientStarted = true;
      if (this.isLocalDev) console.log('🎵 Comfort Chain resumed after external audio stopped');
    }
  }
  ...
  duckAmbientForExternal() {
    if (this.sounds.ambientMusic && !this.sounds.ambientMusic.paused) {
      ...
      if (this.isLocalDev) console.log('🎵 Comfort Chain volume reduced for external audio');
    }
  }
  ...
  restoreAmbientVolume() {
    if (this.sounds.ambientMusic && !this.sounds.ambientMusic.paused) {
      ...
      if (this.isLocalDev) console.log('🎵 Comfort Chain volume restored');
    }
  }

  // Preload all sounds for better performance
  preloadSounds(force = false) {
    if (this.hasInitialized && !force) {
      return;
    }
    if (this.isLocalDev) {
      console.log(force ? '♻️ Reinitializing audio assets...' : '🔄 Preloading all audio files...');
    }
    // Reinitialize sounds in case they failed initially
    this.initializeSounds(force);
  }
  ...
  testAmbientMusic() {
    ...
    if (sound) {
      ...
      if (sound.paused) {
        ...
        console.log('✅ Comfort Chain is already playing');
      }
    } else {
      console.error('❌ Ambient music sound object not found!');
      console.log('🔄 Attempting to reinitialize...');
      this.initializeSounds(true);
    }
  }
